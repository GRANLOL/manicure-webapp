...
export function checkConfirmation() {
    ...
    const isMasterValid = store.useMasters ? (store.selectedMaster !== null) : true;

    if (store.selectedService && isMasterValid && store.selectedDate && store.selectedTime && isPhoneValid && isNameValid) {
        if (tg.MainButton) {
