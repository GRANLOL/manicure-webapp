...
В репозитории лежит чистый шаблон без чужих данных:

```bash
cp /opt/client2/config.template.json /opt/client2/config.json
```

Откройте его и заполните нужное:

```bash
nano /opt/client2/config.json
```

Минимально нужно заполнить:
- `salon_name` — имя клиента
- `address` и `map_url` — адрес и ссылка на карту
- `timezone_offset` — смещение часового пояса клиента
- `working_hours` — рабочие часы

Остальное (логотип, название в шапке WebApp и т.д.) можно настроить потом через **админ-панель** прямо в боте.

Если база не нужна — создастся новая пустая автоматически.
